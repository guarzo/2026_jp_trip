---
day_number: 12
title: "Mt. Fuji Day Tour (Kawaguchiko)"
date_display: "Wed, July 29"
location: "Tokyo → Mt. Fuji → Tokyo"
highlights:
  - Mt. Fuji Views
  - Lake Kawaguchi
  - Chureito Pagoda
  - Guided Tour
reservations:
  - name: Mt. Fuji guided day tour
    note: BOOKED — Jul 29, 7:00 AM hotel pickup, guide Yoshi, ~11 hours, paid
---

<div class="alert alert-warning">
<p class="alert-title">Booked Guided Tour — 7:00 AM Start (Guide: Yoshi)</p>
<p>This is a full-day private guided tour (~11 hours) with guide <strong>Yoshi</strong>, covering Lake Kawaguchi, Mt. Fuji, and a pagoda viewpoint. Pickup is from the hotel (Metropolitan Ikebukuro). The early start is ideal — mornings give the best odds of seeing the mountain before summer haze and afternoon cloud roll in.</p>
</div>

<div class="alert alert-info">
<p class="alert-title">About the tour name</p>
<p>The listing is titled "…Pagoda and Cherry blossoms!" — that's just the tour's stock name. Cherry blossoms are a spring thing, so don't expect them in late July. What applies to our date: the Lake Kawaguchi views, Mt. Fuji, and the Chureito Pagoda viewpoint.</p>
</div>

## Before You Go

<div class="activity-block">
<div class="activity-time">🌅 Early — Hotel Pickup at 7:00 AM</div>

<div class="activity-card">
<h3>Tour Pickup</h3>
<p><strong>Booked:</strong> July 29, 7:00 AM hotel pickup · guide Yoshi · ~11 hours · paid in full.</p>
<p><strong>Confirm before the day:</strong></p>
<ul>
<li>Exact meeting point at the hotel and how early to be in the lobby</li>
<li>What's included (lunch? entry fees? ropeway?) vs. what to bring cash for</li>
<li>Yoshi's contact number in case of a morning mix-up</li>
</ul>
<p><strong>Eat first:</strong> Grab a konbini breakfast the night before or early that morning — a 7:00 AM departure comes quick.</p>
</div>

</div>

## What You'll See

<div class="activity-block">
<div class="activity-time">☀️ The Stops</div>

<div class="activity-card">
<h3>Lake Kawaguchi</h3>
<p>The most accessible of the Fuji Five Lakes and one of the best close-up views of the mountain. On a calm morning you may catch Fuji reflected in the water.</p>
<div class="activity-meta">
<a href="https://maps.google.com/?q=Lake+Kawaguchi">📍 Lake Kawaguchi</a>
</div>
</div>

<div class="activity-card">
<h3>Chureito Pagoda</h3>
<p>The classic postcard shot: a five-story pagoda with Mt. Fuji rising behind it. There's a stair climb up to the viewpoint — worth it for the iconic photo.</p>
<div class="activity-meta">
<a href="https://maps.google.com/?q=Chureito+Pagoda">📍 Chureito Pagoda</a>
</div>
</div>

<div class="activity-card">
<h3>Mt. Fuji Views</h3>
<p>The tour is built around the best vantage points for the mountain. Many Fuji tours also stop at spots like Oishi Park, the 5th Station (road and weather permitting), or Oshino Hakkai — your guide will confirm the day's route based on conditions.</p>
</div>

</div>

## Heat & Weather

<div class="activity-block">
<div class="activity-time">🌡️ Be Ready</div>

<div class="activity-card">
<h3>Dress for the Lakes</h3>
<p>It's cooler at the lakes than in central Tokyo, but still a hot July day with lots of sun exposure at open viewpoints.</p>
<ul>
<li>Sunscreen, hat, water bottle</li>
<li>Comfortable shoes (Chureito Pagoda steps)</li>
<li>A light layer for AC on the bus / breezy lakeshore</li>
<li>Camera / phone fully charged</li>
</ul>
</div>

<div class="alert alert-info">
<p class="alert-title">If Mt. Fuji is hidden</p>
<p>Summer is the haziest season and the peak is often shy — it happens, and it's nobody's fault. The lakes, pagoda, and countryside still make a beautiful day. Check a "Mt Fuji Kawaguchiko live camera" the night before to set expectations.</p>
</div>

</div>

## Evening

<div class="activity-block">
<div class="activity-time">🌙 Evening — Back in Tokyo</div>

<div class="activity-card">
<h3>Return (~6:00 PM, give or take)</h3>
<p>An ~11-hour tour from 7:00 AM lands you back in Tokyo in the early evening. You'll be tired — grab an easy dinner near the hotel.</p>
<p><strong>Tomorrow is departure day</strong> — get a head start on packing tonight!</p>
<ul>
<li>Organize cards (sleeved, toploaded) and check luggage space</li>
<li>Valuable cards go in carry-on (NEVER checked luggage!)</li>
<li>Keep receipts together for customs</li>
</ul>
</div>

</div>

---

## Reservations

<div class="alert alert-warning">
<p class="alert-title">Mt. Fuji Guided Tour — BOOKED & PAID</p>
<p><strong>July 29 · 7:00 AM hotel pickup · guide Yoshi · ~11 hours.</strong> Re-confirm the meeting point and inclusions a day or two ahead. No train booking needed — transport is part of the tour.</p>
</div>

---

## What to Bring

- [ ] Tour confirmation (printed or on phone)
- [ ] Sunscreen + hat
- [ ] Water bottle
- [ ] Light layer (bus AC / lake breeze)
- [ ] Comfortable shoes (pagoda steps)
- [ ] Cash for lunch/snacks/souvenirs (confirm what the tour includes)
- [ ] Camera / phone charged

---

## Why This Day Works

- Someone else handles all the driving and logistics — easy after a busy trip
- Early 7:00 AM start beats the worst of the summer haze
- Hits the iconic views (Lake Kawaguchi + Chureito Pagoda) in one day
- Relaxed evening return sets up an easy departure day tomorrow
